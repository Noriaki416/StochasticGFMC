  weight[0] = 0.0;
  weight[1] = p.C+0.5*(p.Jz+p.h);
  weight[2] = p.C+0.5*(p.Jz+p.h);
  weight[3] = 0.0;
  weight[4] = 0.0;
  weight[5] = p.C;
  weight[6] = p.C+p.h;
  ...
  // 0 1 -> 0 1
  // 0 1 -> 0 1    weight: p.C+0.5*(p.Jz+p.h)
  ...
  prob1[1][0] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[1][0] = prob1[1][0] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  prob1[1][1] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[1][1] = prob1[1][1] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  ...
  prob1[1][2] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[1][2] = prob1[1][2] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  prob1[1][3] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[1][3] = prob1[1][3] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  // vkind: 2 inleg: 0 outleg: 0 vkind_after_flipp: 2
  // 1 0 -> 1 0
  // 1 0 -> 1 0    weight: p.C+0.5*(p.Jz+p.h)
  ...
  prob1[2][0] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[2][0] = prob1[2][0] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  ...
  prob1[2][1] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[2][1] = prob1[2][1] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  prob1[2][2] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[2][2] = prob1[2][2] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  ...
  prob1[2][3] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[2][3] = prob1[2][3] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  prob1[3][0] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[3][0] = prob1[3][0] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  prob1[3][1] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[3][1] = prob1[3][1] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  ...
  // 0 1 -> 0 1
  // 1 0 -> 0 1    weight: p.C+0.5*(p.Jz+p.h)
  ...
  prob1[3][2] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[3][2] = prob1[3][2] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  ...
  prob1[3][3] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[3][3] = prob1[3][3] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  prob1[4][0] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[4][0] = prob1[4][0] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  ...
  prob1[4][1] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[4][1] = prob1[4][1] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  // 1 0 -> 1 0
  // 0 1 -> 1 0    weight: p.C+0.5*(p.Jz+p.h)
  ...
  prob1[4][2] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[4][2] = prob1[4][2] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  prob1[4][3] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[4][3] = prob1[4][3] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  ...
  // 1 1 -> 0 1
  // 1 1 -> 0 1    weight: p.C+0.5*(p.Jz+p.h)
  ...
  prob1[5][0] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[5][0] = prob1[5][0] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  // 1 1 -> 1 0
  // 1 1 -> 1 0    weight: p.C+0.5*(p.Jz+p.h)
  ...
  prob1[5][1] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[5][1] = prob1[5][1] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  prob1[5][2] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[5][2] = prob1[5][2] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  prob1[5][3] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  prob2[5][3] = prob1[5][3] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C);
  ...
  // 0 0 -> 1 0
  // 0 0 -> 1 0    weight: p.C+0.5*(p.Jz+p.h)
  ...
  prob1[6][0] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[6][0] = prob1[6][0] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  ...
  // 0 0 -> 0 1
  // 0 0 -> 0 1    weight: p.C+0.5*(p.Jz+p.h)
  ...
  prob1[6][1] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[6][1] = prob1[6][1] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  ...
  prob1[6][2] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[6][2] = prob1[6][2] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  ...
  prob1[6][3] = (p.t)/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
  prob2[6][3] = prob1[6][3] + (p.C+0.5*(p.Jz+p.h))/(p.t+p.C+0.5*(p.Jz+p.h)+p.C+p.h);
