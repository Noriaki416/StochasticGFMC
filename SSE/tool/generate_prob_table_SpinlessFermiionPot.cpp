...
int main() {
  ...
  v[1].s[0] = 0; v[1].s[1] = 1; 
  v[1].s[2] = 0; v[1].s[3] = 1;
  v[1].weight = "p.C[i]+0.5*(p.v[i]+p.h[i])"; v[1].priority = 2; 
  v[2].s[0] = 1; v[2].s[1] = 0; 
  v[2].s[2] = 1; v[2].s[3] = 0; 
  v[2].weight = "p.C[i]+0.5*(p.v[i]+p.h[ir[i]])"; v[2].priority = 2;
  v[3].s[0] = 0; v[3].s[1] = 1; 
  v[3].s[2] = 1; v[3].s[3] = 0;
  v[3].weight = "p.t[i]"; v[3].priority = 1;
  v[4].s[0] = 1; v[4].s[1] = 0; 
  v[4].s[2] = 0; v[4].s[3] = 1;
  v[4].weight = "p.t[i]"; v[4].priority = 1;
  v[5].s[0] = 1; v[5].s[1] = 1; 
  v[5].s[2] = 1; v[5].s[3] = 1;
  v[5].weight = "p.C[i]"; v[5].priority = 3;
  v[6].s[0] = 0; v[6].s[1] = 0; 
  v[6].s[2] = 0; v[6].s[3] = 0;
  v[6].weight = "p.C[i]+0.5*(p.h[i]+p.h[ir[i]])"; v[6].priority = 4;
  ...
  */
  ...
  ofstream ofs;
  ofs.open("prob_table_SpinlessFermionPot.cpp");  
  ofs << "  for (int i=0; i<p.nsite; i++) {" << endl
      << "    weight[0][i] = 0.0;" << endl
      << "    weight[1][i] = " << v[1].weight << ";" << endl
      << "    weight[2][i] = " << v[2].weight << ";" << endl
      << "    weight[3][i] = 0.0;" << endl
      << "    weight[4][i] = 0.0;" << endl
      << "    weight[5][i] = " << v[5].weight << ";" << endl
      << "    weight[6][i] = " << v[6].weight << ";" << endl
      << "  }" << endl;
  ...
  for (int i=1; i<=6; i++) {
	...
	ssum = ssum + result[j].weight + "+";
      ...
      ofs << "  for (int i=0; i<p.nsite; i++) {" << endl
	  << "    prob1[" << i << "][" << ileg << "][i] = (" << result[0].weight << ")/(" << ssum << ");" << endl;
      if (count_nonzero_weight == 3) {
	ofs << "    prob2[" << i << "][" << ileg << "][i] = prob1[" << i << "][" << ileg << "][i] + ("<< result[1].weight << ")/(" << ssum << ");" << endl;
      } else {
	ofs << "    prob2[" << i << "][" << ileg << "][i] = 1.0;" << endl;
      }
      ofs << "  }" << endl;
    }
  }
